...
Public Function sqlUpdateCaseStatement(rs0 As Recordset, table_name As String, set_columns As String, where_columns As String, dbtype0 As dbType, Optional force_string As Boolean = False) As String
    ...
    'NEW: Initialize case statements
    ...
    'NEW: Process recordset
    With rs0
        Do While Not .EOF
            'NEW: Construct where condition for current record
            ...
            'NEW: Construct set conditions for each column
            For i = LBound(set_case_statements) To UBound(set_case_statements)
            ...
            'NEW: Check if batch is full or if it's the last record
            If c Mod N = 0 Or .EOF Then
                'NEW: Complete the CASE statements
                For i = LBound(set_case_statements) To UBound(set_case_statements)
                    set_case_statements(i) = set_case_statements(i) & " END"
                Next i

                'NEW: Construct the full SQL update statement
                ...
                'NEW: Append to the final SQL statement
                If (c <= N) Then
                    sql1 = sql0
                Else
                    sql1 = sql1 & db.MSSQL_LINE_BREAK & sql0
                ...
                'NEW: Reset case statements for the next batch
                For i = LBound(set_case_statements) To UBound(set_case_statements)
                    If i = LBound(set_case_statements) Then
                       set_case_statements(i) = "SET " & Trim(set_column_array(i)) & " = CASE"
                    Else
    ...
    'NEW: Return the constructed SQL update statement
